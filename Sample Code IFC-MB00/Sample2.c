...
//	Project description	: Sample2 for IFC-MB00 with Control Panel
//  Date				: 21 April 2008
//
//=====================================================================================
...
//	The first 4 functions prototype is fixed, user is also advised not to
//	change it.
//=====================================================================================
void ISRHigh(void);
void ISRLow(void);
	...
	while(1)
	{
		...
		if(cp1_sw(1)==0)					// Test whether SW1 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				// Display Thank You note when SW1 is pressed
				cp1_goto(0,0);					// set the position of message to be displayed
				cp1_str("Thanks for using");	// send message to be displayed by calling function cp1_str
				cp1_goto(1,0);					// set the position of message to be displayed
				cp1_str("Cytron Product");		// send message to be displayed by calling function cp1_str
			}
		}
		...
		if(cp1_sw(2)==0)					// Test whether SW2 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				// user need to press reset button on MB00 or CP04 to exit from the mode
				// Display Decimal Number when SW2 is pressed
				cp1_goto(0,0);					// set the position of message to be displayed 
				cp1_str("Decimal Number");		// send message to be displayed by calling function cp1_str
				cp1_goto(1,0);					// set the position of message to be displayed 
				cp1_dec(123456,6);				// send decimal to be displayed by calling function cp1_dec
			}
		}
		...
		if(cp1_sw(3)==0)					// Test whether SW3 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				// user need to press reset button on MB00 or CP04 to exit from the mode
				// Display Character when SW3 is pressed
				cp1_goto(0,0);					// set the position of message to be displayed 
				cp1_str("Character");			// send message to be displayed by calling function cp1_str
				cp1_goto(1,0);					// set the position of message to be displayed 
				cp1_char('A');					// send character to be displayed by calling function cp1_char	
			}
		}
		...
		{
			cp1_clr();						// clear LCD screen by calling function cp1_clr
			led4 = 1;						// turn ON LED4 on MB00 by sending value '1'
			buzzer = 1;						// buzzer ON for first time
			delay(50000);					// delay for buzzer ON time
			buzzer = 0;						// OFF buzzer
			delay(50000);					// delay for buzzer OFF time
			...
			buzzer = 0;						// OFF buzzer
			delay(50000);					// delay for buzzer OFF time
			buzzer = 1;						// buzzer ON for forth times
			delay(50000);					// delay for buzzer ON time
			buzzer = 0;						// OFF buzzer
			while(1)
			{
				// this is an infinity loop after SW4 on IFC-CP04 is pressed, 
				// user need to press reset button on MB00 or CP04 to exit from the mode
				// Display Binary Number when SW1 is pressed
				cp1_goto(0,0);					// set the position of message to be displayed 
				cp1_str("Binary Number");		// send message to be displayed by calling function cp1_str
				cp1_goto(1,0);					// set the position of message to be displayed 
				cp1_bin(0b11101100,8);			// send binary number to be displayed by calling function cp1_bin
			}
		}
	}
}
